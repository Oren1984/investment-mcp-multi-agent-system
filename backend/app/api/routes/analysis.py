...
def _run_crew_sync(run_id: str, ticker: str, period: str) -> None:
    # run_id_var is already set in the copied context — logs will carry run_id
    gateway = get_gateway()
    crew = InvestmentCrew(gateway=gateway)
    config = AnalysisConfig(ticker=ticker, run_id=run_id, period=period)
    crew.run(config)


async def _run_crew_background(run_id: str, ticker: str, period: str) -> None:
    run_id_var.set(run_id)
    loop = asyncio.get_event_loop()
    # copy_context propagates ContextVars (including run_id_var) into the executor thread
    ctx = copy_context()
    await loop.run_in_executor(_executor, ctx.run, _run_crew_sync, run_id, ticker, period)
...
async def create_analysis(
    ...
    run = await repo.create(
        ticker=body.ticker,
        config={"period": body.period},
    )
    logger.info("Analysis created", extra={"run_id": run.id, "ticker": run.ticker})
